...
LR_calculator_cross <- function(simulation_population, ref.pop, num_contrib, num_sims, is.a.truecontrib, seed){
...
# Set random seed for simulation, default is 1 
# set.seed(opt$rand_seed)
...
for (ref.pop in pops.for.sims) {
  if (opt$pop != ref.pop) {
    ...
    results <- LR_calculator_cross(pop_fp, ref_fp, num_contrib, num_sims, is.a.truecontrib, seed)
    print(pop_fp)
    print(ref_fp)
    print(results)
...
if (is.a.truecontrib==1){
  filename = paste0(opt$out_prefix, '/LRCross_', pop_label, '_seed_', 
                    opt$rand_seed,'_',num_sims, '_', num_contrib,'_contribINmix.rdsw')
}else{
  filename = paste0(opt$out_prefix, '/LRCross_', pop_label, '_seed_', 
