...
fpr_plot <- {
  ...
  ylab("False Positive Rate") + theme(legend.position = c(0.25,0.80),
  legend.text = element_text(size =12),text = element_text(size = 12),
  ...
  legend.key.height = unit(2, "pt")) + scale_fill_manual(values = c("#BCE4D8","#BCE4D8","#BCE4D8","#BCE4D8",
  "#BCE4D8","#BCE4D8")) + geom_point(data = fpr_combined_df, aes(x=as.numeric(contribs), y = value, color = variable)) +
  geom_line(data = fpr_combined_df, aes(x=as.numeric(contribs), y = value, color = variable)) +
  labs(colour = "Genetic Diversity", size = 1)  + scale_color_manual(labels = c("0.674", "0.741", "0.773",
  "0.779", "0.784", " 0.787", " 0.796", "0.804"), values = rainbow(8)) + guides(fill = "none") + 
  scale_y_continuous(trans = "log10", labels = label_number(accuracy = 0.0001)) + 
  labs(y=expression("False Positive Rate"), x = expression("Number of Contributors")) 
}
# view final plot 
fpr_plot 
