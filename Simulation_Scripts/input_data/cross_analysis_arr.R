...
num_sims = opt$nsims
pop = paste0('LTA_Jan2023_Scripts/', opt$pop)
...
for (ref.pop in pops.for.sims) {
  if (pop != ref.pop) {
    ...
    sim_pop= pop
    ref.pop = paste0('LTA_Jan2023_Scripts/', ref.pop)
    results <- LR_calculator_cross(sim_pop, ref.pop, num_contrib, num_sims, is.a.truecontrib)
    print(results)
    print(sim_pop, ref.pop)
    print(cross_pairs)
...
position = unlist(gregexpr("_", pop))
x = position[1] - 1
filename = substr(pop, 1, x)
